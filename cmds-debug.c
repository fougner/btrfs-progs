...
static const char * const debug_cmd_group_usage[] = {
	"btrfs debug [<group>] <command> [<args>]",
	NULL
};

static const char * const cmd_debug_dump_tree_usage[] = {
	"btrfs debug dump-tree [options] device",
...
static int cmd_debug_dump_tree(int ac, char **av)
{
	...
	while(1) {
		...
		switch(c) {
			...
			case 't':
				...
				break;
			default:
				usage(cmd_debug_dump_tree_usage);
		}
	}
...
static const char debug_cmd_group_info[] =
"overall debug tasks and information";

const struct cmd_group debug_cmd_group = {
	debug_cmd_group_usage, debug_cmd_group_info, {
		{ "dump-tree", cmd_debug_dump_tree,
			cmd_debug_dump_tree_usage, NULL, 0 },
